...
// Modification de la hauteur du texte pour le mettre en valeur
void SetTextHeight(osgText::Text* text, float & coefficient) {

    if (HeightPara) {
        osg::Vec3 textPos, position;
        float coef;
        textPos = text->getPosition()* M * V * P * W;
       coef=coefficient;

        position=text->getPosition();
        position.z() = Heightbase + 100*coef;
...
osg::ref_ptr<osgText::Text> ShowParaStudyInfo(osg::ref_ptr<osgText::Text> text,
	...
	ParaStudyInfo->setFont("${workspace_loc:/${ProjName}/Vera.ttf");
...
osg::Vec4 ComputeScreenBound(osg::ref_ptr<osgText::Text> tx) {
	osg::BoundingBox bbox = tx->computeBoundingBox();
	...
	if (dataset->getLength() == position->getLength()) {
		...
		for (XMLSize_t i = 0; i < nodeCount; i++) {
					...
					&& datasetNode->getNodeType() == DOMNode::ELEMENT_NODE) // is element
			{
				...
				text->setBackdropType(osgText::Text::OUTLINE);
				text->setBackdropColor(osg::Vec4f(0.8f,1.0f,1.0f,1.0f));
...
//		text = dynamic_cast<osgText::Text*>(d.get());
        ...
        //Résout le pb de texte qui tremble, mais il faut pointer l'emplacement sur la carte
        //et non le texte
        if (HeightPara) {
            ObjTextPos.z()=Heightbase;
        }
		...
		SetTextHeight(text, coefficient);
