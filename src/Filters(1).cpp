...
float objSinusoidFilter(float distance) {
	...
	if ((0 < x) && (x < v)) {
		objSinusoid = (M_PI * x)/v - M_PI/2;;
		objSinusoid = sin(objSinusoid)*u/2 + u/2;
	} else {
		objSinusoid = u;
	}
...
float objPiecewiseFilter(float camtextdistance) {
	...
	if (x <= 0.69) {
		...
		objPiecewise = (5.45 - 5 * x)/2;
	} else if (x <= 0.9) {
		objPiecewise = (8.55 - 8 * x)/3;
	} else if (x <= 0.979) {
		objPiecewise = (368.55 - 370 * x)/79;
	} else if (x <= 0.986) {
		objPiecewise = (4.224 - 4 * x)/7;
	} else {
		objPiecewise = 0;
	}
...
float objOrderingFilter(float NearestDistance, float NearTextDistance) {
	...
	float tempSize = 0.0;
	float a = (NearestDistance*42 / 13)/MaxDistance;
	tempSize = (a + 0.36 / 13)*2;

	//tempSize is used to calculate the ordering coefficient
	objOrdering = tempSize*((0.5 * NearTextDistance)/2000);
	return objOrdering;
}
...
float scrLinearFilter(float ScrDistance) {
	...
	float scrlinear = ScrDistance / sqrt(a + b);
...
float scrEllipseFilter(osg::Vec3 Posi) {
	...
	float value1 = ((x - ScrCenterX) * (x - ScrCenterX))/(a*a);
	float value2 = ((y - ScrCenterY) * (y - ScrCenterY))/(b*b);
...
float scrCosinusFilter(float ScrDistance) {
	...
	float ratio = ScrDistance / sqrt(ScrCenterX * ScrCenterX + ScrCenterY * ScrCenterY);
...
//Screen space parametrised personal filter
float scrMyFilter(float ScrDistance) {
	float scrMy = 1;
	float reduction = 0.6;
	float width = 10;
	...
	if (ratio <= width) {
		scrMy = scrMy - reduction
				- (reduction / 2) * log10(ratio);
	} else {
		scrMy = scrMy - reduction;
	}
...
float scrStdCosinFilter(float ScrDistance) {
	...
	float x = ScrDistance / sqrt(ScrCenterX * ScrCenterX + ScrCenterY * ScrCenterY);

	float scrstdcosin = cos(M_PI * x / 2);
...
float CombinedFiltersEffect(float Obj, float Scr, float Nearest, float NearText,
	...
	if (scrMyFilterPara) {
		finalfilter = finalfilter * scrMyFilter(Scr);
	}
